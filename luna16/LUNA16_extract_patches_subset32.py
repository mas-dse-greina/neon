...
def SavePatches(manifestFilename_trans, 
...
def extract_candidates(img_file):
	...
	for candidate_idx in range(numCandidates): # Iterate through all candidates
		...
		mask_width = 16 # This is really the half width so window will be double this width
		mask_height = 16 # This is really the half height so window will be double this height
		mask_depth = 16 # This is really the half depth so window will be double this depth
...
manifestFilename_trans = 'manifest_{}_augmented.csv'.format(SUBSET)
...
for root, dirs, files in os.walk(DATA_DIR+SUBSET):
    ...
    for file in files:
        ...
        if (file.endswith('.mhd')) & ('__MACOSX' not in root):  # Don't get the Macintosh directory
            ...
            SavePatches(manifestFilename_trans, 
                        img_file, patchesArray_trans, patchesArray_sag, patchesArray_cor, valuesArray)
